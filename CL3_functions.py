...
# def _init_shutter(com_port="COM12"):
#     shutter = ik.thorlabs.SC10.open_serial(com_port, baud=9600)
#     # set to Manual mode = 1
#     # shutter.mode = 1
#     # check is shutter open and if so, close it
#     if shutter.enable != False:
#         print('shutter was open! I just closed it')
#         shutter.sendcmd('ens')
#     return shutter

def _init_arduino(com_port="COM10"):
    arduino = serial.Serial(com_port,9600,timeout = 0.1) # timeout = 0.1
    # trigger signal is arduino.write(b'2')
    return arduino

#def _init_XY_stage(com_port="COM12"):
#    connection = Connection.open_serial_port(com_port)
#    device_list = connection.detect_devices()
#    # assuming no joy stick
#    device = device_list[0]
#    axis1 = device.get_axis(1)
#    axis2 = device.get_axis(2)
#    return connection, device, axis1, axis2

def _init_XY_stage():
    apt.list_available_devices()
   
    stage_x = apt.Motor(45273204)
    stage_y = apt.Motor(45273214)
    # stage_x = apt.Motor(45214674)
    # stage_y = apt.Motor(45214174)
...
"""
    ...
    with open('C:/Users/olive/Dropbox/FIM/Scripts/OPOtrigger.exp', 'r') as template:
...
class Exp():
    def __init__(self, init=True):
        ...
        """
        if init:
            # self.shut = _init_shutter()
            # self.mstrig = _init_arduino()
            self.a1,self.a2 = _init_XY_stage()
...
#                              'velocity_unit':Units.VELOCITY_MICROMETRES_PER_SECOND}
    ...
    def prepare(self, exp_name,px,py,dx,dy,velocity):
        ...
        make_exp_file(exp_name,px, py, dx, dy, velocity)
        self.px = px
        self.py = py
        self.dx = dx
        x_current = self.a1.position
        if x_current+dx>90:
            print('this run ROI will exceed the stage limit!!! pleace check')
        self.dy = dy
        self.velocity = velocity/1000
   
    def toggle_shutter(self):
        self.shut.sendcmd('ens')
    def trigger_ms(self):
...
#        self.zstage.move(distance_mm)
    ...
    def do_full_raster(self):
        self.origin = [self.a1.position,self.a2.position]
        lines = int(self.dy/(self.py/1e3)) + 1 # define number of lines
        comp = 0
    ...
    def do_one_raster(self,margin=1.05):
        ...
        # start move in x
        _lib.MOT_SetVelParams(45273204, 0,float(self.velocity),float(self.velocity))
        self.a1.move_by(float(self.dx))
#        self.a1.move_relative(self.dx*margin,
#                              wait_until_idle=False,
#                              velocity=self.velocity,
#                              **self.unit_args)
        self.toggle_shutter()
...
#        while self.a1.is_busy()==True:
        while self.a1.is_in_motion == True:
            continue
        self.toggle_shutter()
        time.sleep(0.03) # approx shutter responce time
        # do dy step. No velocity here, so default max is used
#        y_pos = self.a2.position
       
#        self.a2.move_relative(self.py/1e3,
#                              wait_until_idle=False,
#                              **self.unit_args)
        # flyback
        _lib.MOT_SetVelParams(45273204, 0,10,10)
        print('flyback')
        self.a1.move_by(-float(self.dx))
        time.sleep(0.25)
        while self.a1.is_in_motion == True:
        ...
        self.a2.move_by(float(self.py)/1000)
        while self.a2.is_in_motion == True:
            continue
#        self.a1.move_relative(-self.dx*margin,
...
#        self.zstage.close()
    ...
    def acquire_tile(self, pixel = 0.56, overlap=10, acq_area_x = 10, acq_area_y = 10, x_flip = 1, y_flip = 1):
        ...
        for n,pos in enumerate(locs):
            ...
            tagged_image = self.core.get_image()
            time.sleep(exposure*1e-3)
            pixels = np.reshape(tagged_image,
                                newshape=[self.core.get_image_height(), self.core.get_image_width(),4])
    ...
    def calculate_ROI (self,filename = 'ROI.csv', overlap = 10, tile_width=0.81, tile_height=0.6, x_flip = 1, y_flip = 1, locs = [], grid_X = [], grid_Y = []):
        path = 'optical ROIs/'
        ROI = pd.read_csv(path+str(filename))
        with Image.open(path+'whole_tissue_'+str(datetime.date.today())+'.png') as img:
            img.load()
        ImR = np.array(img)
        image_width = round(ROI['W'][0]/(ImR.shape[0]*(1-overlap/100))*np.max(grid_X)*tile_width,2)
        image_height = round(ROI['H'][0]/(ImR.shape[1]*(1-overlap/100))*np.max(grid_Y)*tile_height,2)
        locs = np.array(locs)
        locs = locs.reshape(np.max(grid_X)+1,np.max(grid_Y)+1,2)
        x_gind = ROI['X'][0]/(ImR.shape[0]*(1-overlap/100))*np.max(grid_X)
        y_gind = ROI['Y'][0]/(ImR.shape[1]*(1-overlap/100))*np.max(grid_Y)
        # print(x_gind)
        # print(y_gind)
        start_tile = locs[int(np.floor(x_gind)),int(np.floor(y_gind))]
        start_pos = [start_tile[0]+x_flip*((x_gind-np.floor(x_gind))-0.5)*tile_width,start_tile[1]+y_flip*((y_gind-np.floor(y_gind))-0.5)*tile_height]
    ...
    def wellplate_read(self,a = 0.1, b = 0.25, start_pos = [], x_flip = 1, y_flip = 1, wells = 96, dwell = 2, direction = 'colwise'):
        ...
        if direction == 'rowwise':
            for n in range(grid_dims[1]):
                for m in range(grid_dims[0]):
                    locs.append([x_start+x_flip*9*m, y_start+y_flip*9*n])
            locs = np.array(locs)
            locs = locs.reshape(8,12,2)
